...
from handlers.main import inline_query, message_handler, show_refferal, show_menu, top_services, show_wallet, show_support
...
from api.sms_api import setup_routes
...
class TelegramBot:
    ...
    async def start_webhook(self) -> Tuple[web.AppRunner, web.TCPSite]:
        """Start the webhook server."""
        app = web.Application()
        
        # Setup routes for both Telegram webhook and SMS API
        app.router.add_post(self.webhook_path, self.handle_webhook)
        await setup_routes(app)  # Setup SMS API routes
    ...
    async def run(self) -> None:
        ...
        async with self.initialize_services():
            if self.services_initialized:
                ...
                if self.use_webhook:
                    ...
                    try:
                        # Start both the bot service and periodic updates
                        await asyncio.gather(
                            asyncio.create_task(self.register_handlers()),
                            asyncio.create_task(periodic_update(update=update))
                        )
                    finally:
                        await runner.cleanup()
                else:
                    # Start both polling mode and periodic updates
                    await asyncio.gather(
                        asyncio.create_task(self.register_handlers()),
                        asyncio.create_task(self.bot.polling()),
                        asyncio.create_task(periodic_update(update=update))
                    )
...
async def main():
    ...
    async with bot.initialize_services():
        if bot.use_webhook:
            ...
            runner, site = await bot.start_webhook()
            try:
                # Start both the bot service and periodic updates
                update_task = asyncio.create_task(periodic_update(update=update))
                handler_task = asyncio.create_task(bot.register_handlers())
                
                await asyncio.gather(handler_task, update_task)
            finally:
                await runner.cleanup()
        else:
            # Start both polling mode and periodic updates
